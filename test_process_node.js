...
// Mock n8n input
const inputJson = JSON.parse(fs.readFileSync('test_gemini_input.json', 'utf8'));

const $input = {
  all: () => inputJson.map(item => ({ json: item }))
};
...
function runNode() {
    ...
    function processOpportunity(opp) {
        ...
        // Allow Persian chars (\u0600-\u06FF), English (a-zA-Z), numbers (0-9), spaces (\s), and basic punctuation (.,-)
        ...
        if (opp.type === 'internal') {
            ...
            // Ensure site: operator is preserved or added if missing (though user prompt adds it)
    ...
    for (const item of items) {
        ...
        if (parsedOpportunities.length > 0) {
            ...
            console.warn("⚠️ No opportunities found in item.");
        }
    }
